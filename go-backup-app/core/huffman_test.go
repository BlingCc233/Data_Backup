...
func TestBitWriterAndReader(t *testing.T) {
	...
	for i, expectedBit := range bitsToWrite {
		bit, err := reader.ReadBit()
		if err != nil {
			t.Fatalf("ReadBit failed at index %d: %v", i, err)
		}
		if bit != expectedBit {
			t.Errorf("Read wrong bit at index %d. Got %v, want %v", i, bit, expectedBit)
		}
	}
	...
	if err == nil {
		t.Error("Expected error when reading past end of bitstream, but got nil")
	}
}
